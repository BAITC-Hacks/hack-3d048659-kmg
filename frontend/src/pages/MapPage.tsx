...
import { formatDate, number, stamp, turbineName } from "../lib/format";
import TurbineMap from "../components/map/TurbineMap";
import HorizonControl from "../components/HorizonControl";
import "./MapPage.css";

type Props = Pick<ForecastWorkspace, "run" | "runs" | "observations" | "turbine" | "changeTurbine" | "horizon" | "setHorizon" | "hour" | "setHour">;

export default function MapPage({ run, runs, observations, turbine, changeTurbine, horizon, setHorizon, hour, setHour }: Props) {
  ...
  const readings = mapReadings(runs, observations, run.issuedAt, time, mode);
  const reading = readings.find((value) => value.id === turbine)!;
  const site = turbineSites.find((value) => value.id === turbine)!;
  ...
  return <section className="map-workspace" aria-label="Карта ветропарка">
    ...
    <TurbineMap readings={readings} turbine={turbine} mode={mode} onSelect={changeTurbine} />
    <div className="map-timeline">
      ...
      </> : <p className="map-empty" role="status">В архиве пока нет измерений. Выберите режим «Прогноз».</p>}
    </div>
    <div className="map-detail" aria-live="polite">
      <div className="map-site"><span className="map-eyebrow">ВЫБРАННЫЙ ОБЪЕКТ</span><h3>{turbineName(turbine)}</h3><p>{site.coordinates[1].toFixed(6)}° N · {site.coordinates[0].toFixed(6)}° E</p></div>
    ...
    <div className="map-footnote">
      <p>{mode === "forecast" ? `Архивный прогноз от ${stamp(run.issuedAt)} UTC. Мощность нормирована от 0 до 1.`
